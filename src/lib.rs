...
//! will be created using `Default::default()`.
//!
...
macro_rules! phoenix_tls {
    ...
    ($(#[$attr:meta])* $vis:vis static $name:ident: $t:ty) => (
        ...
        impl $name {
            #[inline]
            $vis fn get(self) -> $crate::Phoenix<$t> {
                self.with(|x| unsafe {
                    $crate::Phoenix::clone_raw(x.into())
                })
            }
            ...
            $vis fn with<F: FnOnce(&$t) -> O, O>(self, f: F) -> O {
                thread_local!{
                    $(#[$attr])* $vis static __SLOW: $crate::Phoenix<$t> =
                        $crate::Phoenix::new(Some(|| with(|x| x.set(None))));
                }
                ...
                unsafe fn init<F: FnOnce(&$t) -> O, O>(f: F) -> O {
                    match __SLOW.try_with(|x| {
                        ...
                        Some(nn) => f(nn.as_ref()),
                        None => f(&*$crate::Phoenix::<$t>::default())
                    }
                }

                // TLS access through POD is faster.
                thread_local!{
